...
document.addEventListener("DOMContentLoaded", function () {
	...
	if (isUnifiedView) initUnifiedView();
	else initSingleView();
	...
	function setupEventListeners(recordsLabel = 'records') {
		...
		document.body.addEventListener('click', (e)=>{
			...
			if (e.target.closest('#nav-toggle')) toggleNav();
		});
	...
	function filterAndShowRows(searchTerms) {
		...
		for (const row of cached.rows) {
			if (searchTerms.every(t=>row._searchText.includes(t))) {
				rowsToShow.add(row);
				if (!isUnifiedView) {
					sectionsToShow.add(row.closest('section'));
					tablesToShow.add(row.closest('table'));
				}
			}
		}
	...
	function resetVisibility() {
		...
		if (isUnifiedView) {
			cached.thead.hidden = false;
			cached.tabla.hidden = false;
		} else {
	...
	function processAndSortTables(tables) {
		return tables.flatMap(tabla=>{
			...
			return Array.from(tabla.tBodies[0].rows, row=>{
				row._searchText = row.textContent.toLowerCase().trim();
